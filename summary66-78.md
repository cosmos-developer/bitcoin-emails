...
OBOBOB>         enabled in the Linux build that you sent me? Block are
...
> 
>     About how long did it take to start?  It could be the node that you
OBOBOB>     happened to request from is slow.  The slow start is consistent with
...
OBOBOB>     that two copies of Bitcoin not run on the same machine at once
OBOBOB>     because they would be modifying the database at the same time.
>      There is never any need to run two on one machine as coin
OBOBOB>     generation will now use multiple processors automatically.
> 
> 
OBOBOB> The reason I run two instances at the same time is to transfer bitcoins 
...
OBOBOBCc: Liberty Standard <newlibertystandard@gmail.com>
OBOBOBSubject: Re: Linux build ready for testing
> Martti, how long did it take to start downloading blocks when you ran
> it, and how fast did it download?
OBOBOB
...
[O> looking much more attractive now that 6 out of the 10 sets of bitcoins I 
